...
let parse_identifier =
  let* fst =
    ...
    *> satisfy (function
      ...
      | _ -> false)
  in
  let* rest = take_while (fun ch -> Char.is_alphanum ch || Char.is_digit ch || Char.equal ch '_') in
...
let parse_list_pat p = parse_sqparens @@ sep_by1 (parse_token ";") p >>| List.fold_right ~f:pcons ~init:(pconst CNil)
let parse_list_expr p = parse_sqparens @@ sep_by1 (parse_token ";") p >>| List.fold_right ~f:econs ~init:(econst CNil)
...
let parse_match_expr pe =
  let pexpr = parse_token "match" *> pe <* parse_token "with" <* option "" (parse_token "|") in
...
let parse_decl =
  ...
  let parse_dlet =
    lift2
      dlet
      (parse_token "let" *> option NonRec (parse_token "rec" *> return Rec))
      (both parse_pattern (lift2 efunf (many parse_pattern <* parse_token "=") parse_expr))
  in
  parse_dlet
;;
