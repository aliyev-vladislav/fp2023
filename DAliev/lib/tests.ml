...
module ParserTests = struct
  ...
  let%test _ =
    parse_test "let f p = p" [ DLet (NonRec, (PId "f", EFun (PId "p", EId "p"))) ]
  ;;
  ...
  let%test _ =
    parse_test
      ...
      [ DLet
          ...
          , ( PId "v"
            , ECons
                ( EBinOp (Mult, EConst (CInt 2), EConst (CInt 2))
                , ECons
                    ( EBinOp (Add, EConst (CInt 2), EConst (CInt 2))
                    , ECons
                        ( EBinOp (Sub, EConst (CInt 2), EConst (CInt 2))
                        , ECons
                            (EBinOp (Div, EConst (CInt 2), EConst (CInt 2)), EConst CNil)
                        ) ) ) ) )
      ]
  ;;
      ...
      [ DLet
          ...
          , ( PId "fac"
            , EFun
                ( PId "n"
                , EIf
                    ( EBinOp (Lt, EId "n", EConst (CInt 1))
                    , EConst (CInt 1)
                    , EBinOp
                        ( Mult
                        , EId "n"
                        , EApp (EId "fac", EBinOp (Sub, EId "n", EConst (CInt 1))) ) ) )
            ) )
      ]
  ;;
  ...
  let%test _ =
    parse_test
      "let v = match l with | h1 :: h2 :: tl -> if h1 <= h2 then h1 else h2 | h1 :: [] \
       -> h1 | _ -> 0 ;;"
      [ DLet
          ( NonRec
          , ( PId "v"
            , EMatch
                ...
                , [ ( PCons (PId "h1", PCons (PId "h2", PId "tl"))
                    , EIf (EBinOp (Lte, EId "h1", EId "h2"), EId "h1", EId "h2") )
  ...
  let%test _ =
    parse_test
      ...
      [ DLet
          ...
          , ( PId "v"
            , ETuple
                [ ECons (EConst (CInt 1), ECons (EConst (CInt 2), EConst CNil))
                ; ECons (ETuple [ EConst (CInt 3); EConst (CInt 4) ], EConst CNil)
                ] ) )
      ]
  ;;

  let%test _ =
    parse_test "let () = () in ()" [ DLet (NonRec, (PConst CUnit, EConst CUnit)) ]
  ;;
  ...
  let%test _ =
    parse_test
      ...
      [ DLet
          ( NonRec
          , ( PId "f"
            , EFun (PTuple [ PId "e1"; PId "e2" ], EBinOp (Add, EId "e1", EId "e2")) ) )
      ]
  ;;

  let%test _ =
    parse_test
      "let f e1 e2 = f e1 + f e2 ;;"
      [ DLet
          ( NonRec
          , ( PId "f"
            , EFun
                ...
                , EFun
                    ( PId "e2"
                    , EBinOp (Add, EApp (EId "f", EId "e1"), EApp (EId "f", EId "e2")) )
                ) ) )
      ]
  ;;
end
...
module InterpretTests = struct
  ...
  let%expect_test _ =
    interpret_test {|
        let [x; y; z] = [0; 1; 2; 3] ;;
      |};
    [%expect {| Infer error: Expression contains not implemented features |}]
  ;;
